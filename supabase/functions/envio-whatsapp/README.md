...
## Antes de que funcione de verdad: falta un secret

```bash
supabase secrets set WHATSAPP_ACCESS_TOKEN=<token>
```

Sin él, la función responde `503` con un mensaje claro en vez de fallar oscuro. Es un token
...
- El envío real contra la Graph API: **sin verificar** — falta el secret de arriba, y el único
  contacto real (JSC) tiene la ventana cerrada desde el 3 sept, así que tampoco hay con quién
  probar el camino de texto libre hasta que llegue un mensaje entrante nuevo.
